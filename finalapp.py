...
def main():
    ...
    predict_button1 = st.button('Predict Model 1', key='button_model1')
    predict_button2 = st.button('Predict Model 2', key='button_model2')

    # Make prediction when a button is clicked
    if predict_button1:
        # Preprocess input data and make prediction using the model
        prediction = predict(age, business_travel, daily_rate, department, distance_from_home, education,
    ...
    if predict_button2:
        ...
        prediction = predict_attrition(model2, pd.DataFrame(filled_input_data, index=[0]))
        
        st.subheader('Prediction from Model 2')
        if prediction[0] == 1:
...
def predict(age, business_travel, daily_rate, department, distance_from_home, education,
    ...
    status = "Employee is leaving Organisation" if prediction[0] == 1 else "Employee is staying in Organisation"
